...
export default function EventPage() {
  ...
  useEffect(() => {
    ...
    const unsubEvent = onSnapshot(doc(db, "events", id), (d) => { if (d.exists()) setEvent(d.data() as EventData); });
  ...
  const submitAnswer = async () => {
    ...
    try { await addDoc(collection(db, "events", id, "participants"), { name, comment, answers: myAnswers, created_at: serverTimestamp(), }); setName(""); setComment(""); setMyAnswers({}); setChatName(name); alert("登録しました！"); } catch { alert("エラー"); } finally { setIsSubmitting(false); }
  };
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 space-y-12">
        ...
        <div className="flex justify-between items-center border-b border-slate-800 pb-6">
          ...
          <div className="flex gap-0 border border-slate-700">
            <button onClick={() => setIsEditMode(!isEditMode)} className={`px-5 py-2 text-xs font-black tracking-widest transition ${isEditMode ? "bg-orange-500 text-black" : "bg-[#111] hover:bg-[#222] text-slate-400"}`}>
              {isEditMode ? "DONE" : "EDIT"}
            </button>
            <button onClick={copyUrl} className="bg-[#111] hover:bg-[#222] text-slate-400 px-5 py-2 text-xs font-black tracking-widest transition border-l border-slate-700">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          <div className="lg:col-span-8 space-y-8">
            <div>
              ...
              <div className="bg-[#111] border-l-4 border-cyan-500 p-6">
            ...
            {isEditMode && (
              <div className="bg-[#150a05] border border-orange-900/50 p-6 animate-fadeIn">
                ...
                <div className="flex flex-col sm:flex-row gap-0">
                  <input type="date" className="bg-[#000] border border-slate-700 p-3 text-sm text-white outline-none focus:border-orange-500" value={editDate} onChange={(e) => setEditDate(e.target.value)} />
                  <select className="bg-[#000] border border-l-0 sm:border-l-0 border-t-0 sm:border-t border-slate-700 p-3 text-sm text-white outline-none focus:border-orange-500" value={editTime} onChange={(e) => setEditTime(e.target.value)}>
                    {timeOptions.map(t => <option key={t} value={t}>{t}〜</option>)}
                  </select>
                  <button onClick={addCandidate} disabled={!editDate} className="bg-orange-600 text-black px-6 py-3 font-bold text-xs hover:bg-orange-500 uppercase tracking-widest">ADD</button>
                </div>
              </div>
            )}

            {/* 出欠テーブル */}
            <div className="overflow-x-auto border border-slate-800 bg-[#0A0A0A]">
              <table className="w-full border-collapse text-sm whitespace-nowrap">
                <thead>
                  <tr className="bg-[#111] border-b border-slate-800">
                    <th className="p-4 text-left w-40 sticky left-0 z-20 bg-[#111] border-r border-slate-800 text-slate-500 font-bold uppercase text-[10px] tracking-widest">Participant Name</th>
                    {event.candidates.map((c) => {
                      const isBest = bestIds.includes(c.id);
                      return (
                        <th key={c.id} className={`p-3 text-center min-w-[100px] border-l border-slate-800 relative ${isBest && !isEditMode ? "bg-cyan-900/20" : ""}`}>
                    ...
                    <th className="p-4 text-left min-w-[200px] text-slate-500 font-bold uppercase text-[10px] tracking-widest pl-6 border-l border-slate-800">Comment</th>
                  </tr>
                </thead>
                <tbody>
                  {participants.map((p) => (
                    <tr key={p.id} className="border-b border-slate-800 hover:bg-[#161616] transition">
                      <td className="p-4 font-bold text-white sticky left-0 z-10 bg-[#0A0A0A] border-r border-slate-800 truncate max-w-[160px]">
                        {p.name}
                      </td>
                      {event.candidates.map((c) => {
                        const isBest = bestIds.includes(c.id);
                        return (
                          <td key={c.id} className={`p-2 text-center border-l border-slate-800 ${isBest && !isEditMode ? "bg-cyan-900/10" : ""}`}>
                            {renderSymbol(p.answers[c.id])}
                          </td>
                        );
                      })}
                      <td className="p-4 text-slate-500 pl-6 truncate max-w-[200px] font-mono text-xs border-l border-slate-800">{p.comment}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="lg:col-span-4 space-y-10">
            ...
            {/* 入力フォーム */}
            <div className="bg-[#111] p-8 border border-slate-800 hover:border-cyan-900/50 transition duration-300">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <input type="text" className="w-full bg-[#000] border-2 border-slate-700 p-4 text-white placeholder-slate-600 focus:border-cyan-500 outline-none transition-colors" placeholder="山田 太郎" value={name} onChange={(e) => setName(e.target.value)} />
                </div>
                ...
                <div>
                  ...
                  <div className="flex gap-0 border-2 border-slate-700">
                    <button onClick={() => setAllAnswers("o")} className="flex-1 bg-[#111] text-cyan-400 py-3 text-xs font-black hover:bg-cyan-900/20 transition border-r border-slate-700">ALL ◎</button>
                    <button onClick={() => setAllAnswers("t")} className="flex-1 bg-[#111] text-yellow-400 py-3 text-xs font-black hover:bg-yellow-900/20 transition border-r border-slate-700">ALL △</button>
                ...
                <div className="bg-[#000] p-1 space-y-1 max-h-64 overflow-y-auto border-2 border-slate-700 custom-scrollbar">
                  {event.candidates.map((c) => (
                    <div key={c.id} className="flex justify-between items-center bg-[#111] p-3 border border-slate-800">
                      ...
                      <div className="flex gap-1">
                        {[["o","◎","text-cyan-400"], ["t","△","text-yellow-400"], ["x","✕","text-slate-600"]].map(([val, label, color]) => (
                          <button key={val} onClick={() => setMyAnswers({ ...myAnswers, [c.id]: val })} className={`w-8 h-8 text-sm font-black transition ${myAnswers[c.id] === val ? `bg-[#000] border border-slate-600 ${color}` : "text-slate-700 hover:text-slate-500"}`}>{label}</button>
                        ))}
                      </div>
                    </div>
                  ))}
                ...
                <div>
                   ...
                   <input type="text" className="w-full bg-[#000] border-2 border-slate-700 p-4 text-white placeholder-slate-600 focus:border-cyan-500 outline-none transition-colors" placeholder="遅れます等..." value={comment} onChange={(e) => setComment(e.target.value)} />
                </div>
                
                <button onClick={submitAnswer} disabled={isSubmitting} className="w-full bg-cyan-600 text-black font-black text-lg py-5 hover:bg-cyan-500 transition shadow-[0_0_20px_rgba(8,145,178,0.2)] disabled:opacity-50 tracking-widest uppercase">
                  SUBMIT ENTRY
                </button>
              </div>
            </div>
            ...
            {/* チャット */}
            <div className="bg-[#111] p-8 border border-slate-800 flex flex-col h-[500px]">
                ...
                {messages.map((msg) => {
                  ...
                  return (
                    <div key={msg.id} className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                      ...
                      <div className={`relative max-w-[85%] p-4 text-sm font-medium border ${isMe ? "bg-pink-900/20 border-pink-500/50 text-pink-100" : "bg-[#0A0A0A] border-slate-700 text-slate-300"}`}>
                        {msg.text}
              ...
              <div className="flex gap-2 pt-4 border-t border-slate-800">
                <input type="text" placeholder="NAME" className="w-1/3 bg-[#000] border border-slate-700 p-3 text-xs text-white focus:border-pink-500 outline-none" value={chatName} onChange={(e) => setChatName(e.target.value)} />
                <input type="text" placeholder="MESSAGE..." className="flex-1 bg-[#000] border border-slate-700 p-3 text-xs text-white focus:border-pink-500 outline-none" value={chatText} onChange={(e) => setChatText(e.target.value)} onKeyDown={(e)=>e.key==="Enter"&&sendMessage()} />
                <button onClick={sendMessage} className="bg-pink-600 text-black px-5 font-bold hover:bg-pink-500 transition uppercase text-xs tracking-widest">SEND</button>
              </div>
            </div>
